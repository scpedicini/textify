...
pub trait HistoryItem: Clone + PartialEq {
    ...
    /// Approximate heap bytes retained by this item. Containers can use this to bound history.
    ...
    I: HistoryItem,
{
    ...
    /// Set the approximate byte budget for undo items, defaults to unlimited.
...
mod tests {
    ...
    #[test]
    fn test_history_byte_budget_retains_only_the_newest_oversized_item() {
        let mut history: History<TabIndex> = History::new().max_bytes(1);
        history.push(0.into());
        history.push(1.into());
